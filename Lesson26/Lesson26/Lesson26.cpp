...
void DrawWall()										// Draws The Floor
{
	glBindTexture(GL_TEXTURE_2D, texture[0]);			// Select Texture 1 (0)
	glBegin(GL_QUADS);									// Begin Drawing A Quad
		glNormal3f(0.0, 0.0, 1.0);						// Normal Pointing Up
			glTexCoord2f(0.0f, 1.0f);					// Bottom Left Of Texture
			glVertex3f(-2.0, 2.0, 0.0);					// Bottom Left Corner Of Floor

			glTexCoord2f(0.0f, 0.0f);					// Top Left Of Texture
			glVertex3f(-2.0,-2.0, 0.0);					// Top Left Corner Of Floor

			glTexCoord2f(1.0f, 0.0f);					// Top Right Of Texture
			glVertex3f( 2.0,-2.0, 0.0);					// Top Right Corner Of Floor

			glTexCoord2f(1.0f, 1.0f);					// Bottom Right Of Texture
			glVertex3f( 2.0, 2.0, 0.0);					// Bottom Right Corner Of Floor
	glEnd();											// Done Drawing The Quad
}
...
int DrawGLScene(GLvoid)									// Draw Everything
{
	...
	//把模板缓冲区中，地面区域设置成1，抑制R、G、B、Alpha写入帧缓冲区中
...
{
	if (keys[VK_RIGHT])		yrotspeed += 0.005f;			// Right Arrow Pressed (Increase yrotspeed)
	if (keys[VK_LEFT])		yrotspeed -= 0.005f;			// Left Arrow Pressed (Decrease yrotspeed)
	if (keys[VK_DOWN])		xrotspeed += 0.005f;			// Down Arrow Pressed (Increase xrotspeed)
	if (keys[VK_UP])		xrotspeed -= 0.005f;			// Up Arrow Pressed (Decrease xrotspeed)

	if (keys['A'])			zoom +=0.005f;				// 'A' Key Pressed ... Zoom In
	if (keys['Z'])			zoom -=0.005f;				// 'Z' Key Pressed ... Zoom Out

	if (keys[VK_PRIOR])		height +=0.005f;				// Page Up Key Pressed Move Ball Up
	if (keys[VK_NEXT])		height -=0.005f;				// Page Down Key Pressed Move Ball Down
}
